...
version: V3
status: REPAIRED_POST_MERGE_RESEARCH_CANDIDATE_V3
...
pullbacks; verifies every mismatch control; independently exercises deletion of
each of the six contract-key coordinates; and exhaustively checks pair and triple
universal properties over the declared finite domains.
...
## 12. V3 witness-completeness repair

The distinct V2 rereview reproduced the pullback but found that target identity,
version, and semantic-contract digest were not independently varied. V3 adds
three single-coordinate mismatch objects. Every field of the full key now has a
finite erasure witness. This repairs evidence coverage only; it does not enlarge
the theorem or authorize any field's real-world semantics.
