...
\toprule
\textbf{Rival} & \textbf{What it grants} & \textbf{Where it blocks the chain} & \textbf{Standing assessment} \\
\midrule
\textbf{Brute modal primitivism} & Uncreated modal-normative structure (step 5!) & Step 7 (no intelligent ground; structure is bedrock) & Consistent; explanatorily arrears-running — it re-labels the explanandum as primitive. Unrefuted. \\
\textbf{Platonism / necessary abstracta} & Objective, necessary structure & Steps 6–7 (ground is inert abstracta) & Faces the applicability/instantiation problem (why does the concrete \emph{participate}?) and offers no account of faculty-target fit. Unrefuted as ontology; weak as explanation. \\
\textbf{Structural realism} & Objective structure of the actual world & Steps 2→3 read modally (structure may be actual-world-only) & Must still ground the modal force of evaluation (possible-but-not-made placements); tends to collapse into primitivism or Platonism at that point. \\
\textbf{Nominalism / conventionalism} & Concrete particulars + our classifying practices & Step 2 (no antecedent determinability; we impose) & Founders on error: if classification is imposition, unsoundness of an imposition needs a standard the view disallows; sophisticated versions re-import objective similarity, i.e., orthability. \\
\textbf{Selected-function naturalism} & Real (etiological) function and malfunction & §3.3 T1 (teleology reduced to selection) & The serious naturalist option. Answered only conditionally (T1); if T1 fails, the faculty route stops at biology — recorded, not hidden. \\
\textbf{Semantic naturalism} & Naturalized content (information/teleosemantics) & Step 11's richer reading; §3.3 & Same shape as selected-function debate, at the level of meaning; unresolved in general philosophy of mind, and this paper does not pretend otherwise. \\
\textbf{Divine conceptualism} & Intelligent ground of abstracta/logic (Anderson \& Welty 2011) & Nothing before step 8; a fellow traveler that identifies the structure with divine thoughts & Nearest theistic neighbor; differences (attributes vs thoughts; the Atharī paper's refusal to make possibility-space an entity) are doctrinal, handled there. \\
\textbf{Infinitism} & Each level grounded in a prior level, without end & Step 5 (well-foundedness of constitutive dependence) & Blocked only by a no-infinite-regress-of-grounding premise; that premise is defended as the norm in grounding theory but is a premise. Recorded. \\
\textbf{Deflationism about normativity} & The practice of evaluation-talk & Step 1 read realistically & If evaluation-talk is kept truth-apt, C′ follows; if not, the position abandons the datum. The retreat is available and priced (§4.1). \\
\textbf{Generic classical theism (no further identification)} & Steps 1–10 & Nothing; declines step 11's theological development & Fully compatible terminus of this paper; the identification of the ground with the God of a particular revelation is \emph{revelational/school-internal} and is made only in the Atharī companion. \\
\bottomrule
\end{tabular}
\end{center}
