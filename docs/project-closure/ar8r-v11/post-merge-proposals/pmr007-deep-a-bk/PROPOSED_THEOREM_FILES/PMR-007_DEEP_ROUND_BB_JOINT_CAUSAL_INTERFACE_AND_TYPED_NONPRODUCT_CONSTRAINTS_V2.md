# PMR-007 Deep Round BB V2 — joint causal-interface quotients and typed nonproduct constraints
...
Define the finite **raw product-support deficit**
...
cross-interface closure.  It does not yet show that the coordinates were
independently typed, that the full product was a warranted null model, or why
...
common output.  It narrows a fully independent product rival only under the
following independently audited guards:

```text
INTERFACE_REGISTRY_FIXED;
NULL_PRODUCT_WARRANTED;
STATE_SUPPORT_COMPLETE;
ACTION_SCOPE_COMPLETE;
NO_OMITTED_LOAD_BEARING_COORDINATE;
and an explicit coupling/derivation account rather than a duplicated or
definitional interface.
```
...
## 6. Raw-deficit authority ceiling and controls

`D_raw` is invariant under relabeling inside the frozen quotient registry but
is not invariant under duplicating, splitting, coarsening, or replacing the
interface registry.  It is therefore not an architecture score and cannot be
compared across arbitrary decompositions.  Pairwise deficits do not determine
...
## 8. Fable, OSM, PRH, and Deep-Z effect
...
The V1 primary checker passed the formal theorem and simultaneously found
4,374 duplicate-interface systems with positive `D_raw`.  V2 treats this not
as a theorem failure but as decisive evidence that raw nonproduct support is
not, without typed-null guards, explanatory integration.
