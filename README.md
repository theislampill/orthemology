...
- **Candidate overlay (R7C, Decision 0026).** `main` (R6) holds Decisions 0001–0019. Decisions 0020–0027 and their documents/PDFs are **`proposed-candidate` in the unmerged PR chain** (PR #8 → PR #9 → R7C grandchild), all produced by Opus candidate passes and **not merged, and awaiting fresh independent (Fable) review**. The authoritative merged/candidate split is machine-checked in [`docs/project-closure/r7c/CANDIDATE-STATE.yaml`](docs/project-closure/r7c/CANDIDATE-STATE.yaml) (`scripts/validate_candidate_state.py`); promotion to `adopted-merged` is a fresh-Fable-review + protected-merge act, never an Opus self-promotion.
