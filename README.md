...
- **Generated candidate overlay (R7E Sol review, Decisions 0034–0036).** `main` (R6) holds Decisions 0001–0019. Decisions 0020–0036 and their documents/PDFs remain **`proposed-candidate` in the unmerged PR #8 → #9 → #10 → #11 → #12 chain**. The authoritative [`docs/current-candidate-state.yaml`](docs/current-candidate-state.yaml) is generated offline from the timestamped [`CANDIDATE-TOPOLOGY-INPUT.yaml`](docs/project-closure/r7e-sol/CANDIDATE-TOPOLOGY-INPUT.yaml) and records exact branch/base/`head_at_observation`/draft/CI/provenance facts observed at `2026-07-21T18:10:22Z`; those facts are not timeless live-head claims. `scripts/validate_candidate_state.py` rejects incomplete or stale topology, provenance or decision-allocation drift, self-promotion, and evidence-free merge/signoff/readiness claims. No PR is merged, independently signed off, or ready for merge; refresh the observation before any separately authorized protected integration.
