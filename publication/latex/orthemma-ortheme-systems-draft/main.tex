...
% source-sha256: 1f0139907fa54f02e431f95e854089443e2952afb7291dd939a2ef4f2a705fd0
...
% breakable-row-table: data-rows=6 total-rendered-characters=795 max-row-rendered-characters=153
\par\medskip
\hrule height 0.8pt
\smallskip
% breakable-row: 1/6
...
\operatorname{Succ}_a(m) \subseteq M,\\
 \quad \text{each element reached by an edge labeled with the action that produced it,}
\end{multline*}
...
\begin{itemize}
\item $O^*(m; A)$ — actual profile under analysis $A$ (ground truth relative to $A$);

\item $\hat{p}_{A,\alpha,t}(m)$ — the profile actor $\alpha$ infers at time $t$ under $A$;

\item $C_{A,\alpha,t}(m)$ — the candidate structure actor $\alpha$ maintains at time $t$.
...
% breakable-row-table: data-rows=4 total-rendered-characters=1069 max-row-rendered-characters=302
\par\medskip
\hrule height 0.8pt
\smallskip
% breakable-row: 1/4
\noindent\textbf{Column 1}: \textbf{Pattern}\par
...
\smallskip
% breakable-row: 2/4
\noindent\textbf{Column 1}: \textbf{Formally}\par
...
\smallskip
% breakable-row: 3/4
\noindent\textbf{Column 1}: \textbf{Example}\par
...
\smallskip
% breakable-row: 4/4
\noindent\textbf{Column 1}: \textbf{Wrong reduction}\par
...
\begin{multline*}
h = \langle\, \text{channel } k;\ \text{property class } \tau;\\
\ \text{scope } \sigma;\ \text{provenance};\\
\ \text{validity/expiry} \,\rangle
\end{multline*}
