...
% source-sha256: 4243bd98e13f23e305088f99f6649a2fbe598d619721704cfdcd907a7e3ebfdd
...
\begin{itemize}
\needspace{3\baselineskip}\item A person says something, here and now, that reaches a listener as an ambiguous acoustic stream — compatible with ``olive juice'' and with ``I love you.''

\needspace{3\baselineskip}\item A learner meets an Arabic word-token whose segmentation, lemma, morphology, attached clitics, and syntactic role are not yet settled.

\needspace{3\baselineskip}\item A patient arrives vomiting red material; the presentation is compatible with several risk states and several causes.

\needspace{3\baselineskip}\item A build system emits ``all tests pass'' for this commit, which may mean the behaviour was genuinely exercised, or merely that files exist, or that the evidence is stale.
...
\begin{itemize}
\needspace{3\baselineskip}\item \textbf{orthemma} = the concrete situated occurrence;

\needspace{3\baselineskip}\item \textbf{ortheme} = the repeatable operational state-type it instantiates.
...
% breakable-row-table: data-rows=6 total-rendered-characters=799 max-row-rendered-characters=153
\par\medskip
\hrule height 0.8pt
\smallskip
% breakable-row: 1/6
...
\noindent\textbf{\#}: 1\par
\noindent\textbf{Formal addition}: Versioned identity and labeled lineage on the occurrence domain\par
\noindent\textbf{What it closes}: ``right finding, wrong copy''; verdicts silently transported across mutations\par
\smallskip
\hrule height 0.4pt
\smallskip
% breakable-row: 2/6
\needspace{5\baselineskip}
\noindent\textbf{\#}: 2\par
\noindent\textbf{Formal addition}: Analysis / actor / time indices on profiles and candidate sets\par
\noindent\textbf{What it closes}: ground truth only relative to a declared analysis; cross-actor divergence as a stop signal\par
\smallskip
\hrule height 0.4pt
\smallskip
% breakable-row: 3/6
\needspace{5\baselineskip}
\noindent\textbf{\#}: 3\par
\noindent\textbf{Formal addition}: Typed, scoped, expiring evidence channels\par
\noindent\textbf{What it closes}: green-but-mis-scoped validators; stale evidence treated as current\par
\smallskip
\hrule height 0.4pt
\smallskip
% breakable-row: 4/6
\needspace{5\baselineskip}
\noindent\textbf{\#}: 4\par
\noindent\textbf{Formal addition}: Factorized candidate structure with route composition\par
\noindent\textbf{What it closes}: one occurrence, several independent defects, each with its own route\par
\smallskip
\hrule height 0.4pt
\smallskip
% breakable-row: 5/6
\needspace{5\baselineskip}
\noindent\textbf{\#}: 5\par
\noindent\textbf{Formal addition}: Per-burden closure status with residual dispositions\par
\noindent\textbf{What it closes}: false closure as a type error; scalar ``percent complete'' banned\par
\smallskip
\hrule height 0.4pt
\smallskip
% breakable-row: 6/6
\needspace{5\baselineskip}
\noindent\textbf{\#}: 6\par
\noindent\textbf{Formal addition}: Episode reification with a joint verdict vector\par
\noindent\textbf{What it closes}: correct-by-luck placements; pathway adequacy separable from result correctness\par
\smallskip
\hrule height 0.8pt
\par\medskip
...
\begin{itemize}
\needspace{3\baselineskip}\item $O^*(m; A)$ — actual profile under analysis $A$ (ground truth relative to $A$);

\needspace{3\baselineskip}\item $\hat{p}_{A,\alpha,t}(m)$ — the profile actor $\alpha$ infers at time $t$ under $A$;

\needspace{3\baselineskip}\item $C_{A,\alpha,t}(m)$ — the candidate structure actor $\alpha$ maintains at time $t$.
...
\begin{enumerate}
\needspace{3\baselineskip}\item \textbf{Static operational separation.} Force two placements to share an action, and if their best individual actions differ enough, the forced merger incurs excess risk above tolerance — infinite if no common admissible action exists.

\needspace{3\baselineskip}\item \textbf{Predictive separation under a strictly proper score.} If the task is forecasting and two placements imply different conditional distributions, one shared forecast is strictly worse than separate truthful forecasts; predictive difference individuates types only when prediction is the declared task.

\needspace{3\baselineskip}\item \textbf{Finite-horizon safe merger (controlled bisimulation).} States with identical admissible actions, immediate losses, class-transition probabilities, and terminal losses have equal optimal value; the quotient preserves value. This licenses \emph{just-in-time placement}: leave $\hat{p}_t(m)$ coarse now, run a discriminating test, and refine before the consequential action — preservation of a distinction can be procedural, not a persistent label.
...
% breakable-row-table: data-rows=4 total-rendered-characters=1073 max-row-rendered-characters=302
\par\medskip
\hrule height 0.8pt
\smallskip
% breakable-row: 1/4
\needspace{5\baselineskip}
\noindent\textbf{\textbf{Pattern}}\par
\noindent\textbf{Inter-orthemma aliasing}: Distinct occurrences produce the same or confusable observation while instantiating operationally different types\par
\noindent\textbf{Intra-orthemma uncertainty}: One encountered occurrence remains compatible with several candidate types\par
\noindent\textbf{Identity uncertainty}: It is unresolved \emph{which occurrence} (which $\kappa$, or which version $v$) is actually in hand\par
\smallskip
\hrule height 0.4pt
\smallskip
% breakable-row: 2/4
\needspace{5\baselineskip}
\noindent\textbf{\textbf{Formally}}\par
\noindent\textbf{Inter-orthemma aliasing}: $\Omega(m_1) = \Omega(m_2)$, $O^*(m_1; A) \neq O^*(m_2; A)$\par
\noindent\textbf{Intra-orthemma uncertainty}: $C^{\mathrm{profile}} \supseteq \{p_1, \dots, p_k\}$, several profiles licensing non-equivalent routes\par
\noindent\textbf{Identity uncertainty}: $C^{\mathrm{id}}(m) \subseteq M$ non-singleton: candidates for the case's own identity/version\par
\smallskip
\hrule height 0.4pt
\smallskip
% breakable-row: 3/4
\needspace{5\baselineskip}
\noindent\textbf{\textbf{Example}}\par
\noindent\textbf{Inter-orthemma aliasing}: Two jars of white powder look identical; one is flour, one is cornstarch\par
\noindent\textbf{Intra-orthemma uncertainty}: One acoustic stream compatible with ``olive juice'' and ``I love you''\par
\noindent\textbf{Identity uncertainty}: The log line may describe this commit's build or the previous one's; the file at this path may be the original or a reused slot\par
\smallskip
\hrule height 0.4pt
\smallskip
% breakable-row: 4/4
\needspace{5\baselineskip}
\noindent\textbf{\textbf{Wrong reduction}}\par
\noindent\textbf{Inter-orthemma aliasing}: Treating it as one case with an open profile\par
\noindent\textbf{Intra-orthemma uncertainty}: Treating it as two cases\par
\noindent\textbf{Identity uncertainty}: Treating it as profile uncertainty about a known case\par
\smallskip
\hrule height 0.8pt
\par\medskip
...
% breakable-row-table: data-rows=5 total-rendered-characters=635 max-row-rendered-characters=179
\par\medskip
\hrule height 0.8pt
\smallskip
% breakable-row: 1/5
\needspace{5\baselineskip}
\noindent\textbf{Family}: $C^{\mathrm{id}}$\par
\noindent\textbf{Ranges over}: $M$\par
\noindent\textbf{Open question}: \emph{which occurrence} (identity/version) is in hand — Section 3's third column\par
\smallskip
\hrule height 0.4pt
\smallskip
% breakable-row: 2/5
\needspace{5\baselineskip}
\noindent\textbf{Family}: $C^{\mathrm{profile}}$\par
\noindent\textbf{Ranges over}: $\Pi_A$ (profile space)\par
\noindent\textbf{Open question}: \emph{which profile} the case instantiates — competing hypotheses may themselves be whole profiles, never coerced into single orthemes\par
\smallskip
\hrule height 0.4pt
\smallskip
% breakable-row: 3/5
\needspace{5\baselineskip}
\noindent\textbf{Family}: $C^{\mathrm{cause}}$\par
\noindent\textbf{Ranges over}: $\mathcal{K}_A$ (cause repertoire)\par
\noindent\textbf{Open question}: \emph{which cause} produced the state\par
\smallskip
\hrule height 0.4pt
\smallskip
% breakable-row: 4/5
\needspace{5\baselineskip}
\noindent\textbf{Family}: $C^{\mathrm{route}}$\par
\noindent\textbf{Ranges over}: $\mathcal{R}_A$ (route repertoire)\par
\noindent\textbf{Open question}: \emph{which operation/owner} should receive the case\par
\smallskip
\hrule height 0.4pt
\smallskip
% breakable-row: 5/5
\needspace{5\baselineskip}
\noindent\textbf{Family}: $C^{\mathrm{warrant}}$\par
\noindent\textbf{Ranges over}: $\mathcal{W}_A$ (warrant states)\par
\noindent\textbf{Open question}: \emph{which warrant state} obtains — authorized, established, both, neither\par
\smallskip
\hrule height 0.8pt
\par\medskip
...
\begin{itemize}
\needspace{3\baselineskip}\item the \textbf{true profile} $O^*(m; A) \in \Pi_A$ — one complete profile, fixed by the occurrence and the analysis, independent of anyone's evidence;

\needspace{3\baselineskip}\item the \textbf{candidate set} $\hat{C}_{A,\alpha,t}(m) \subseteq \Pi_A$ (the typed family $C^{\mathrm{profile}}$) — the set of complete profiles the actor has not yet ruled out; correctness of maintenance means $O^*(m; A) \in \hat{C}$ whenever the evidence so far is veridical, and investigation shrinks $\hat{C}$;

\needspace{3\baselineskip}\item the \textbf{inferred partial profile} $\hat{p}_{A,\alpha,t}(m) \in \Pi_A^\partial$ — what the actor currently \emph{places}: determined on the resolved axes, open on the rest. A partial profile corresponds to the set of its completions, so $\hat{p}$ and $\hat{C}$ are inter-constrained ($\hat{C} \subseteq \operatorname{completions}(\hat{p})$ when both are maintained honestly) — but \textbf{a candidate set is not one inferred profile}: $\hat{C} = \{p_1, p_2\}$ with two live complete profiles is a different epistemic state from a single vaguer $\hat{p}$, and collapsing the former into the latter loses exactly the alternatives structure that routes and discriminating tests act on;

\needspace{3\baselineskip}\item optional \textbf{belief weights} — a distribution over $\hat{C}$ (or over an axis's alternatives). Weights are \emph{permitted, never required} (Section 5.1), and an unweighted candidate set is a legitimate terminal representation, not an unfinished one.
...
\begin{itemize}
\needspace{3\baselineskip}\item factors with disjoint operational footprints compose freely (treat the identity defect \emph{and} correct the quantity defect);

\needspace{3\baselineskip}\item where composed routes conflict, declared priority applies, and \textbf{hard constraints dominate}: a safety constraint on one factor overrides a near-optimal route on another;

\needspace{3\baselineskip}\item an unresolved factor contributes no route, but may contribute a containment obligation (Section 7) that composes like a route.
...
\begin{itemize}
\needspace{3\baselineskip}\item $g$ is the \textbf{governed component} — which part of the mapping/handling machinery this distinction governs (Section 6.2);

\needspace{3\baselineskip}\item $S_\mu$ is the set of \textbf{declared competing higher-order states} the governing context may occupy, specified \emph{in advance}, not read off one incident (e.g., \{appearance-grade, provenance-grade\} for evidence; \{current, stale\} for version);

\needspace{3\baselineskip}\item $\operatorname{select}_\mu$ is the \textbf{selecting evidence} procedure that determines which state in $S_\mu$ actually obtains;

\needspace{3\baselineskip}\item $\operatorname{prov}(\mu) = \langle \mathrm{authority}, \mathrm{warrant}, \mathrm{scope}, \operatorname{ver}(\mu)\rangle$ is the rule's own provenance — a rule of unknown provenance is a stale-evidence problem one level up;

\needspace{3\baselineskip}\item $\operatorname{ver}(\mu)$ is the rule's version, recorded per episode so audits can scope which placements ran under which edition;

\needspace{3\baselineskip}\item $\pi_\mu$, the meta-policy, is the \textbf{conduct rule} that consults the configuration and prescribes behaviour conditional on the obtaining state (``quarantine on stale,'' ``never place on appearance alone'').
...
\begin{itemize}
\needspace{3\baselineskip}\item \textbf{O — repertoire:} which state-types exist and may be posited or retired;

\needspace{3\baselineskip}\item \textbf{I — individuation:} what counts as the same case; identity keys, versioning, lineage;

\needspace{3\baselineskip}\item \textbf{E — evidence:} what evidence counts, of which property class, at what grade;

\needspace{3\baselineskip}\item \textbf{D — disclosure:} what may be left open, and how open questions must be declared (uncertainty handling);

\needspace{3\baselineskip}\item \textbf{R — routing:} where cases are sent, and what happens when the correct route is unavailable;

\needspace{3\baselineskip}\item \textbf{V — validation/closure:} what may be called done, at what standard, and when a completion claim must be reopened;

\needspace{3\baselineskip}\item \textbf{W — warrant-classification:} which warrant states exist (authorized vs factually established and their combinations) and what each licenses.
...
% breakable-row-table: data-rows=5 total-rendered-characters=746 max-row-rendered-characters=149
\par\medskip
\hrule height 0.8pt
\smallskip
% breakable-row: 1/5
...
\begin{itemize}
\needspace{3\baselineskip}\item \textbf{non-disclosure} — the system never notices that $m$ requires a distinction;

\needspace{3\baselineskip}\item \textbf{orthemic underdetermination} — several consequential candidates remain open;

\needspace{3\baselineskip}\item \textbf{misidentification} — $m$ is placed under the wrong ortheme;

\needspace{3\baselineskip}\item \textbf{under-segmentation} — several orthemes in $O^*(m; A)$ are collapsed;

\needspace{3\baselineskip}\item \textbf{over-segmentation} — distinctions are assigned that $m$ does not support;

\needspace{3\baselineskip}\item \textbf{misplacement} — a valid description is attached to the wrong occurrence, component, level, or temporal version (the lineage failures of Section 2.5, including review-transport: a verdict formed about one artifact carried by the review process onto another);

\needspace{3\baselineskip}\item \textbf{representational failure} — the correct ortheme is recognised but cannot be encoded;

\needspace{3\baselineskip}\item \textbf{misrouting} — the correct ortheme is assigned but $m$ is sent to the wrong operation;

\needspace{3\baselineskip}\item \textbf{validation failure} — the placement or repair is not genuinely tested;

\needspace{3\baselineskip}\item \textbf{false closure} — $m$ is treated as fully resolved though consequential parts of its profile or its burden ledger remain open.
...
\emph{unresolved}, \emph{deferred}, \emph{transferred}, \emph{owner-assigned}, \emph{risk-accepted}, or \emph{validated-resolved},
...
\begin{itemize}
\needspace{3\baselineskip}\item \textbf{a formal trigger} — a recurring error class attributable to the current distinction or rule, not a single incident (one miss licenses a hypothesis, not a revision);

\needspace{3\baselineskip}\item \textbf{a complexity penalty} — the revised rule or repertoire pays for its added distinctions against out-of-sample benefit (Definition 9's over-segmentation costs apply to rules as well as types);

\needspace{3\baselineskip}\item \textbf{hysteresis} — revision and reversal thresholds differ, so the repertoire does not churn on boundary noise;

\needspace{3\baselineskip}\item \textbf{impact-scoped reopening} — $\rho$ increments $\operatorname{ver}(\mu)$ and computes the set of \emph{affected} prior episodes: exactly those whose placement consulted the revised state-distinction or selector where the two editions disagree. Only those episodes' evidence status is downgraded from validated to provisional pending re-check. Blanket reopening of everything that ever ran under the old edition is rejected as unscoped;

\needspace{3\baselineskip}\item \textbf{a second-order variant} — when a \emph{pattern across occurrences} (not any single case) shows the state space itself was under-specified, revision enters an enumeration mode: the alternatives are listed exhaustively against the record rather than patched one neighbor at a time. The synthetic worked example of Section 11.4 illustrates such a tranche: serial one-neighbor-per-review revision escalating to enumeration after repeated single-case patches fail to converge.
...
\begingroup\fontsize{6}{7}\selectfont
\begin{verbatim}
...
% breakable-row-table: data-rows=2 total-rendered-characters=1699 max-row-rendered-characters=1117
\par\medskip
\hrule height 0.8pt
\smallskip
% breakable-row: 1/2
\needspace{5\baselineskip}
\noindent\textbf{\textbf{Result correct (V1)}}\par
...
% breakable-row: 2/2
\needspace{5\baselineskip}
\noindent\textbf{\textbf{Result incorrect (¬V1)}}\par
...
\begin{itemize}
\needspace{3\baselineskip}\item \textbf{V1 holds:} the placed claim (``compliant'') is true.

\needspace{3\baselineskip}\item \textbf{V2a arguably holds by the letter:} the declared standard — marker presence — was met.

\needspace{3\baselineskip}\item \textbf{V2b-P fails at the default sensitivity criterion:} over its declared reference class, had an artifact been non-compliant with the marker present (a tool version that always prints it; a spoofed log), the procedure as configured would still have supported the claim — the configured procedure is not truth-conducive. (\textbf{V2b-T also fails as a result-side diagnosis:} this claim was correct, but not \emph{through} the truth-relevant mechanism — the stopped-clock signature.)

\needspace{3\baselineskip}\item \textbf{V6 fails:} the perturbation neighborhood exposes it directly — compliant-without-marker is rejected; non-compliant-with-marker is passed. The paired probes are exactly the metamorphic fixture pair.

\needspace{3\baselineskip}\item \textbf{V3a fails:} the governing evidence configuration is inadequate for the risk class — its selecting evidence cannot discriminate the states it is supposed to separate.
...
Nothing in the episode signature or the verdicts quantifies over awareness. Any rule-governed, evidence-updating executor — a predictive-text decoder resolving an ambiguous keypress sequence, a CI validator, a triage pipeline, a review chain, an institution — executes orthing, and its episodes bear all \emph{applicable} verdicts. Stopped-clock analogues arise mechanically: a decoder emits the right word from a frequency prior that would misfire on the neighboring input — correct message, defective pathway, no conscious subject anywhere. The actor field $\alpha$ is therefore populated with the mechanical executor — never left empty — and executor identity is distinct from any warrant or authority index. Semantic depth — ordered from hard-coded check, through model-based inference over declared alternatives, to reflective capacity to propose rule revisions — is a real axis of the \emph{executor}, orthogonal to every verdict on the \emph{episode}: shallow executors can run adequate pathways and deep ones defective pathways. Depth matters operationally in one place only: which revisions the executor may perform locally versus must escalate across the governance boundary.
...
\begin{itemize}
\needspace{3\baselineskip}\item \textbf{handoff} — a projection of one episode's output (a partial profile, an evidence item with its scope, a route, a disposition) appears in the input or evidence of the next; handoffs are the loci of transport error — a verdict crossing an edge without its scope and version is how stale evidence propagates;

\needspace{3\baselineskip}\item \textbf{supersession} — a later episode revises an earlier one's conclusions;

\needspace{3\baselineskip}\item \textbf{retry-of} — a later episode re-attempts an earlier one's task.
...
\begin{enumerate}
\needspace{3\baselineskip}\item \textbf{one case, one analysis} — all sub-episodes address $m$ or its successor-lineage under a single declared analysis $A$ (hence one task $T = \operatorname{task}(A)$; sub-episodes sharing a task but differing in tolerance, representation, or boundary do NOT compose without an explicitly declared fusion analysis);

\needspace{3\baselineskip}\item \textbf{a declared boundary} — governance names the composite actor (pipeline, team, institution) and the composite governing configuration, including precedence across sub-episode rules;

\needspace{3\baselineskip}\item \textbf{a declared fusion rule} — the composite placement, evidence-status map, and residual ledger are a stated aggregation of sub-outputs (e.g., profile union with per-claim status equal to the weakest status along its supporting path; merged ledgers with surviving ownership) — well-defined, not read off the last node.
...
\needspace{3\baselineskip}\item $\mathcal{G}_{\alpha,A_\alpha}$ is \textbf{not a metaortheme}: it names no governed component and declares no competing higher-order states about the machinery.
...
\begin{itemize}
\needspace{3\baselineskip}\item \textbf{Compatibility:} $Compat_m(\mathcal{G}_1, \mathcal{G}_2)$ iff $\exists m' \in \operatorname{Reach}(m): O^*(m'; A_1) \in \mathcal{G}_1 \wedge O^*(m'; A_2) \in \mathcal{G}_2$ — one shared occurrence, evaluated under each analysis separately;

\needspace{3\baselineskip}\item \textbf{Conflict:} $Conflict_m(\mathcal{G}_1, \mathcal{G}_2)$ iff no such reachable $m'$ exists — zero-sum chess targets conflict by construction;

\needspace{3\baselineskip}\item \textbf{Cooperation (special case):} a shared target profile, $\mathcal{G}_{\alpha_1,A} = \mathcal{G}_{\alpha_2,A}$ under one shared analysis — the actors may still differ in evidence, roles, and routes (that is what the distributed-episode machinery of Section 9.2 is for), but their targets coincide.
...
\begin{itemize}
\needspace{3\baselineskip}\item \textbf{supports incremental value} — both frozen primary contrasts favor the treatment at or beyond their minimum important effects, with no harm rule firing;

\needspace{3\baselineskip}\item \textbf{does not yet support incremental value} — an adequately powered run in which neither the supports nor the harm rule fires (an adequately powered null IS this outcome);

\needspace{3\baselineskip}\item \textbf{evidence of harm or failure} — accuracy degradation or structure-for-structure's-sake beyond the frozen harm margins;

\needspace{3\baselineskip}\item \textbf{inconclusive} — only via a predeclared gate (failed-run rate, power floor, or an outcome-affecting recorded deviation), with the gate named.
...
% breakable-row-table: data-rows=3 total-rendered-characters=771 max-row-rendered-characters=446
\par\medskip
\hrule height 0.8pt
\smallskip
% breakable-row: 1/3
\needspace{5\baselineskip}
\noindent\textbf{Tier}: \textbf{Conceptual/definitional}\par
\noindent\textbf{Content}: The proposed distinctions and typed relationships of Sections 2–10; the split metaortheme normal form; verdict separability; the DAG/composition conditions; the derived multi-actor extensions\par
\noindent\textbf{Status}: Machine-checked internal agreement over the declared definitions, schemas, examples, and adversarial fixtures (deterministic); \textbf{not} a proof of mathematical consistency or completeness; establishes no empirical claim\par
\smallskip
\hrule height 0.4pt
\smallskip
% breakable-row: 2/3
\needspace{5\baselineskip}
\noindent\textbf{Tier}: \textbf{Public observational}\par
\noindent\textbf{Content}: —\par
\noindent\textbf{Status}: \textbf{None currently supplied.} The private records that motivated the design are non-evidential design provenance (Section 17)\par
\smallskip
\hrule height 0.4pt
\smallskip
% breakable-row: 3/3
\needspace{5\baselineskip}
\noindent\textbf{Tier}: \textbf{Experimental}\par
\noindent\textbf{Content}: Machinery benefit (13.1); episode-reification delta (13.2); vocabulary utility (13.3)\par
\noindent\textbf{Status}: \textbf{None.} All three designed, none run\par
\smallskip
\hrule height 0.8pt
\par\medskip
...
\begin{itemize}
\needspace{3\baselineskip}\item Chen, T. Y., Cheung, S. C., \& Yiu, S. M. (1998). \emph{Metamorphic testing: a new approach for generating next test cases.} Technical Report HKUST-CS98-01, Department of Computer Science, HKUST.

\needspace{3\baselineskip}\item Chen, T. Y., Kuo, F.-C., Liu, H., Poon, P.-L., Towey, D., Tse, T. H., \& Zhou, Z. Q. (2018). Metamorphic testing: a review of challenges and opportunities. \emph{ACM Computing Surveys}, 51(1), 4:1–4:27. doi:10.1145/3143561

\needspace{3\baselineskip}\item Chow, C. K. (1970). On optimum recognition error and reject tradeoff. \emph{IEEE Transactions on Information Theory}, 16(1), 41–46. doi:10.1109/TIT.1970.1054406

\needspace{3\baselineskip}\item Crutchfield, J. P., \& Young, K. (1989). Inferring statistical complexity. \emph{Physical Review Letters}, 63(2), 105–108. doi:10.1103/PhysRevLett.63.105

\needspace{3\baselineskip}\item El-Yaniv, R., \& Wiener, Y. (2010). On the foundations of noise-free selective classification. \emph{Journal of Machine Learning Research}, 11, 1605–1641.

\needspace{3\baselineskip}\item Fisher, R. A. (1922). On the mathematical foundations of theoretical statistics. \emph{Philosophical Transactions of the Royal Society A}, 222, 309–368. doi:10.1098/rsta.1922.0009

\needspace{3\baselineskip}\item Geifman, Y., \& El-Yaniv, R. (2017). Selective classification for deep neural networks. \emph{NeurIPS 30}.

\needspace{3\baselineskip}\item Geifman, Y., Uziel, G., \& El-Yaniv, R. (2019). Bias-reduced uncertainty estimation for deep neural classifiers. \emph{ICLR}. (Derivation of AURC/E-AURC.)

\needspace{3\baselineskip}\item Gettier, E. L. (1963). Is justified true belief knowledge? \emph{Analysis}, 23(6), 121–123. doi:10.1093/analys/23.6.121

\needspace{3\baselineskip}\item Givan, R., Dean, T., \& Greig, M. (2003). Equivalence notions and model minimization in Markov decision processes. \emph{Artificial Intelligence}, 147(1–2), 163–223. doi:10.1016/S0004-3702(02)00376-4

\needspace{3\baselineskip}\item Goldman, A. I. (1979). What is justified belief? In G. S. Pappas (Ed.), \emph{Justification and Knowledge} (pp. 1–23). D. Reidel. doi:10.1007/978-94-009-9493-5\_1

\needspace{3\baselineskip}\item Hendrycks, D., \& Gimpel, K. (2017). A baseline for detecting misclassified and out-of-distribution examples in neural networks. \emph{ICLR}.

\needspace{3\baselineskip}\item Hopcroft, J. (1971). An n log n algorithm for minimizing states in a finite automaton. In \emph{Theory of Machines and Computations} (pp. 189–196). Academic Press.

\needspace{3\baselineskip}\item Howard, R. A. (1966). Information value theory. \emph{IEEE Transactions on Systems Science and Cybernetics}, 2(1), 22–26. doi:10.1109/TSSC.1966.300074

\needspace{3\baselineskip}\item International Auditing and Assurance Standards Board (2009). \emph{ISA 330: The Auditor's Responses to Assessed Risks.} IFAC.

\needspace{3\baselineskip}\item International Organization for Standardization (2018). \emph{ISO 31000:2018 Risk management — Guidelines.}

\needspace{3\baselineskip}\item Kaelbling, L. P., Littman, M. L., \& Cassandra, A. R. (1998). Planning and acting in partially observable stochastic domains. \emph{Artificial Intelligence}, 101(1–2), 99–134. doi:10.1016/S0004-3702(98)00023-X

\needspace{3\baselineskip}\item Larsen, K. G., \& Skou, A. (1991). Bisimulation through probabilistic testing. \emph{Information and Computation}, 94(1), 1–28. doi:10.1016/0890-5401(91)90030-6

\needspace{3\baselineskip}\item Littman, M. L., Sutton, R. S., \& Singh, S. (2001). Predictive representations of state. \emph{NIPS 14}.

\needspace{3\baselineskip}\item Moreau, L., \& Missier, P. (Eds.) (2013). \emph{PROV-DM: The PROV Data Model.} W3C Recommendation, 30 April 2013. https://www.w3.org/TR/prov-dm/

\needspace{3\baselineskip}\item Merriam-Webster. “Orth-.” \emph{Merriam-Webster.com Dictionary.} https://www.merriam-webster.com/dictionary/orth- (accessed 26 July 2026).

\needspace{3\baselineskip}\item Merriam-Webster. “-eme.” \emph{Merriam-Webster.com Dictionary.} https://www.merriam-webster.com/dictionary/-eme (accessed 26 July 2026).

\needspace{3\baselineskip}\item Smyth, H. W. (1920). \emph{A Greek Grammar for Colleges.} American Book Company, §§841.2, 861.2.

\needspace{3\baselineskip}\item Liddell, H. G., Scott, R., \& Jones, H. S. \emph{A Greek-English Lexicon}, s.v. \texttt{ἐμός}.

\needspace{3\baselineskip}\item Constable, P. (1999, June 17). “Re: Amerindian Characters.” \emph{Unicode Mail List Archive.}

\needspace{3\baselineskip}\item Burridge, J., \& Blaxter, T. (2020). Using spatial patterns of English folk speech to infer the universality class of linguistic copying. \emph{Physical Review Research}, 2, 043053, p. 24. doi:10.1103/PhysRevResearch.2.043053

\needspace{3\baselineskip}\item Scheirer, W. J., Rocha, A., Sapkota, A., \& Boult, T. E. (2013). Toward open set recognition. \emph{IEEE TPAMI}, 35(7), 1757–1772. doi:10.1109/TPAMI.2012.256

\needspace{3\baselineskip}\item Shalizi, C. R., \& Crutchfield, J. P. (2001). Computational mechanics: pattern and prediction, structure and simplicity. \emph{Journal of Statistical Physics}, 104, 817–879. doi:10.1023/A:1010388907793

\needspace{3\baselineskip}\item Silla, C. N., \& Freitas, A. A. (2011). A survey of hierarchical classification across different application domains. \emph{Data Mining and Knowledge Discovery}, 22, 31–72. doi:10.1007/s10618-010-0175-9

\needspace{3\baselineskip}\item Traub, J., Bungert, T. J., Lüth, C. T., Baumgartner, M., Maier-Hein, K. H., Maier-Hein, L., \& Jaeger, P. F. (2024). Overcoming common flaws in the evaluation of selective classification systems. \emph{NeurIPS 37}. arXiv:2407.01032

\needspace{3\baselineskip}\item Tsoumakas, G., \& Katakis, I. (2007). Multi-label classification: an overview. \emph{International Journal of Data Warehousing and Mining}, 3(3), 1–13. doi:10.4018/jdwm.2007070101

\needspace{3\baselineskip}\item Wald, A. (1947). \emph{Sequential Analysis.} John Wiley \& Sons.

\needspace{3\baselineskip}\item Wetzel, L. (2018). Types and Tokens. In E. N. Zalta (Ed.), \emph{The Stanford Encyclopedia of Philosophy.} https://plato.stanford.edu/entries/types-tokens/
...
\begin{itemize}
\needspace{3\baselineskip}\item \textbf{Orthemma.} A concrete situated occurrence considered as something to be apprehended, carrying an identity key and version. The token pole.

\needspace{3\baselineskip}\item \textbf{Ortheme.} A repeatable operational state-type an orthemma instantiates, relative to a declared, versioned analysis. The type pole. Task-relative wording is shorthand only after one analysis with that task has been fixed.

\needspace{3\baselineskip}\item \textbf{Instantiation relation $\operatorname{Inst}_A$.} $(m, o) \in \operatorname{Inst}_A$: $m$ instantiates $o$ under the declared, versionable analysis $A$ (with $T = \operatorname{task}(A)$); the fibre $O^*(m; A)$ is the actual profile. $\operatorname{Inst}_T$ / $O^*_T(m)$ are licensed local shorthand once a single $A$ with $\operatorname{task}(A) = T$ has been fixed; forbidden in multi-analysis contexts (Definition 3).

\needspace{3\baselineskip}\item \textbf{Typed candidate families $C^{\mathrm{id}}, C^{\mathrm{profile}}, C^{\mathrm{cause}}, C^{\mathrm{route}}, C^{\mathrm{warrant}}$.} Open alternatives per uncertainty axis, with exclusivity marking and an evidence-to-resolve clause.

\needspace{3\baselineskip}\item \textbf{Placement / apprehension.} Assigning an inferred profile / the whole encounter-to-disposition process.

\needspace{3\baselineskip}\item \textbf{Route-sufficient apprehension.} Enough of the profile to route safely while other axes stay open; ≠ identity-complete resolution.

\needspace{3\baselineskip}\item \textbf{Labeled successor set $\operatorname{Succ}_a(m)$.} The zero, one, or many occurrences an action creates; placement validity does not transport across an edge without re-evidence.

\needspace{3\baselineskip}\item \textbf{Metaorthemma (candidate term; object adopted by Decision 0002).} The episode-local, case-bound configuration TOKEN of a metaortheme: $\operatorname{MetaInst}(\bar{\mu}, \mu)$; binds case-specific values (reference frame, tolerance value, instrument + calibration, fixture, success surface, scope) within the declared analysis ($\operatorname{Compatible}(\bar{\mu}, A(e))$); its binder-with-warrant is distinct from the executor; judged by V3c; omitted where no material binding exists. The WORD remains a benchmark-gated candidate (ordinary-language equivalent: ``instantiated governing-configuration token''); the OBJECT is part of the episode record.

\needspace{3\baselineskip}\item \textbf{Metaortheme.} A metaorthemic configuration $\langle g, S_\mu, \operatorname{select}_\mu, \operatorname{prov}(\mu), \operatorname{ver}(\mu)\rangle$ paired with a separable meta-policy; governed component in \{repertoire, individuation, evidence, disclosure, routing, validation, warrant-classification\}; objectives, the task, and the governance meta-level excluded.

\needspace{3\baselineskip}\item \textbf{Residual disposition.} Per burden: unresolved / deferred / transferred / owner-assigned / risk-accepted / validated-resolved.

\needspace{3\baselineskip}\item \textbf{False closure.} A completion claim collapsing other dispositions into ``validated-resolved''; a type error against the burden ledger.

\needspace{3\baselineskip}\item \textbf{Orthing / orthing episode.} The process type of rule-governed, evidence-updating apprehension-and-handling / one dated, situated, record-valued run of it, bearing the verdict vector.

\needspace{3\baselineskip}\item \textbf{Verdict vector (V1–V6, registry-normalized).} Result: V1 correctness, V2b-T factive truth linkage. Pathway core: V2a support; V2b-P procedure reliability; V2c currentness; V3a configuration, V3b policy, V3c token-binding, V3d execution, V3e decision-time adequacy; V4a route safety; V5 closure truthfulness; V6 robustness. Advisory: V4b route quality. Semantic IDs and aliases: \texttt{docs/\allowbreak{}verdict-\allowbreak{}registry.\allowbreak{}yaml} (Decision 0004).

\needspace{3\baselineskip}\item \textbf{Generalized ANDON event.} A governed interruption when continuation or closure is not warranted under the current placement, evidence, route, warrant, or ledger.

\needspace{3\baselineskip}\item \textbf{Target profile $\mathcal{G}_{\alpha,A_\alpha}$.} The state-types an actor aims to make some successor occurrence instantiate, under the actor's own analysis $A_\alpha$; distinct from the descriptive profile, from the objective, and from any metaortheme.
