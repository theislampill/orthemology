...
structure IdentityAndUnityLift where
  ...
  section : World → Token
  sectionCarries : ∀ world, worldCommon world (section world)
  transportCoherent : Prop
  sameBearer : World → Token → World → Token → Prop
  bID : ∀ world₁ world₂,
    sameBearer world₁ (section world₁) world₂ (section world₂)
...
theorem identityAndUnityLift_has_bID
    ...
    ∀ world₁ world₂,
      certificate.sameBearer world₁ (certificate.section world₁)
        world₂ (certificate.section world₂) := by
