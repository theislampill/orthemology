...
out={"schema":"AR8R_PMR007_POST_BV_CONSOLIDATED_RECHECK_V1","seed":SEED,"results":{}}
...
ci={"profiles":0,"content_sensitive":0,"content_insensitive":0,"matched_twins":20000,"twin_failures":0}
...
# Twins are constructed with exact same profiles and differing target expansions.
ci["pass"]=ci["twin_failures"]==0 and ci["content_insensitive"]==25
out["results"]["CI"]=ci

# CJ: binary C,F,P exact ternary-grid output probabilities.
cj={"profiles":0,"fittingness_sensitive":0,"payoff_independent":0,"both":0,"personal_impersonal_twins":20000,"twin_failures":0}
# index = ((c*2+f)*2+p)
...
cj["pass"]=cj["twin_failures"]==0 and cj["both"]>0
out["results"]["CJ"]=cj

out["overall_pass"]=all(v.get("pass",False) for v in out["results"].values())
path=Path(__file__).with_name('post_bv_consolidated_recheck_results.json')
