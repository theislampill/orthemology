...
% source-sha256: 1be1009438c3abd7c73670218e18d7564e97ff76948bb03f503cfa7bd38fe13d
% source: theory/orthemic-multi-actor-conflict-note.md
% source-sha256: 4f00ce4cb26c7a6c884df598c8832fd72b11bc3a35dc287b62ea64d181fa8ff6
...
\]
Here $\iota_n$ is injective, and $\iota_n(e)$ is ``the episode $e$, considered as a case to be audited.''
...
% breakable-row-table: data-rows=23 total-rendered-characters=5199 max-row-rendered-characters=1651
\par\medskip
\hrule height 0.8pt
\smallskip
% breakable-row: 1/23
...
\noindent\textbf{Gloss}: the governing rules in force, with $\preceq$ their declared precedence order\par
\smallskip
\hrule height 0.4pt
\smallskip
% breakable-row: 13/23
...
\begin{enumerate}
\needspace{3\baselineskip}\item \textbf{Episode ≠ output.} $\operatorname{out}(e) = \langle \hat{p}, r, \operatorname{estatus}, \mathcal{Q}, \delta, a \rangle$ is a projection of $e$; episodes with identical outputs can differ in pathway and hence in every §4 verdict.

\needspace{3\baselineskip}\item \textbf{Episode ≠ policy.} $\pi$ is repeatable and undated; $e$ is one dated execution of $\pi$. Reliability claims attach to $\pi$ (and $\vec{\mu}$); correctness claims attach to $e$.

\needspace{3\baselineskip}\item \textbf{Placement inside an episode ≠ the episode.} The compact function style becomes the derived judgment
...
\begin{itemize}
\needspace{3\baselineskip}\item \textbf{(A) Higher-order STATE/TYPE.} A metaortheme is an ortheme \emph{of the governing context}: the context instantiates one of several competing higher-order states (e.g., evidence is provenance-grade or appearance-grade), and that state governs.

\needspace{3\baselineskip}\item \textbf{(B) Higher-order handling RULE.} A metaortheme is a repeatable rule over the machinery: ``never place on appearance alone,'' ``quarantine on stale version.'' The rule \emph{is} the object.

\needspace{3\baselineskip}\item \textbf{(C) Split model.} A metaortheme is a \textbf{metaorthemic configuration} $\mu = \langle g, S_\mu, \operatorname{select}_\mu, \operatorname{prov}(\mu), \operatorname{ver}(\mu)\rangle$ — governed component $g$, declared competing higher-order states $S_\mu$, selecting-evidence procedure $\operatorname{select}_\mu$ that determines which state obtains, provenance, version — together with a distinct \textbf{meta-policy} $\pi_\mu$ prescribing conduct conditional on the state (gloss: the \emph{distinction the rule consults} and the \emph{rule that consults it} are two objects, as Def. 9 already insists in prose).
...
% breakable-row-table: data-rows=5 total-rendered-characters=1365 max-row-rendered-characters=287
\par\medskip
\hrule height 0.8pt
\smallskip
% breakable-row: 1/5
...
= \{\operatorname{V2a},\operatorname{V2b-P},\\
\operatorname{V2c},
\operatorname{V3a},\operatorname{V3b},\\
\operatorname{V3c},
\operatorname{V3d},\operatorname{V3e},\\
\operatorname{V4a},
...
\]
For the compact display below only, write $R_e := \operatorname{ReqPath}(e)$ and $S_i(e):=\operatorname{Status}_i(e)$.
...
% breakable-row-table: data-rows=9 total-rendered-characters=1063 max-row-rendered-characters=165
\par\medskip
\hrule height 0.8pt
\smallskip
% breakable-row: 1/9
\needspace{5\baselineskip}
\noindent\textbf{Relation}: $\operatorname{V2b-T}_q(e) \to \operatorname{V1}_q(e)$\par
\noindent\textbf{Status}: \textbf{Definitional entailment} (factive, claim-wise)\par
\smallskip
\hrule height 0.4pt
\smallskip
% breakable-row: 2/9
\needspace{5\baselineskip}
\noindent\textbf{Relation}: $\operatorname{V2b-P}(e) \to \operatorname{V1}(e)$\par
\noindent\textbf{Status}: \textbf{No} — non-factive by construction\par
\smallskip
\hrule height 0.4pt
\smallskip
% breakable-row: 3/9
\needspace{5\baselineskip}
\noindent\textbf{Relation}: $\operatorname{V3e}(e) \to \operatorname{V1}(e)$\par
\noindent\textbf{Status}: \textbf{No} — decision-time reasonableness is result-free\par
\smallskip
\hrule height 0.4pt
\smallskip
% breakable-row: 4/9
\needspace{5\baselineskip}
\noindent\textbf{Relation}: $\operatorname{PathwayAdequate}(e) \to \operatorname{V1}(e)$\par
\noindent\textbf{Status}: \textbf{No} — the core excludes V1 and V2b-T\par
\smallskip
\hrule height 0.4pt
\smallskip
% breakable-row: 5/9
\needspace{5\baselineskip}
\noindent\textbf{Relation}: $\operatorname{V1}(e) \to \operatorname{PathwayAdequate}(e)$\par
\noindent\textbf{Status}: \textbf{No} — stopped-clock and defective-binding cases\par
\smallskip
\hrule height 0.4pt
\smallskip
% breakable-row: 6/9
\needspace{5\baselineskip}
\noindent\textbf{Relation}: $\operatorname{V1}(e)$ at the governed level vs claim-wise $\operatorname{V1}_q(e)$\par
\noindent\textbf{Status}: Definitional: profile-level V1 aggregates the placed claims per the declared level\par
\smallskip
\hrule height 0.4pt
\smallskip
% breakable-row: 7/9
\needspace{5\baselineskip}
\noindent\textbf{Relation}: $\operatorname{PathwayAdequate}(e) \leftrightarrow \neg \operatorname{PathwayDefective}(e)$\par
\noindent\textbf{Status}: \textbf{Not asserted} — \texttt{undetermined} is a third state\par
\smallskip
\hrule height 0.4pt
\smallskip
% breakable-row: 8/9
\needspace{5\baselineskip}
\noindent\textbf{Relation}: V6 vs V2b-P\par
\noindent\textbf{Status}: \textbf{No entailment either way} (distinct questions; shared evidence possible)\par
\smallskip
\hrule height 0.4pt
\smallskip
% breakable-row: 9/9
\needspace{5\baselineskip}
\noindent\textbf{Relation}: All other pairs\par
\noindent\textbf{Status}: No definitional implication introduced by these definitions; empirical correlation is never entailment\par
\smallskip
\hrule height 0.8pt
\par\medskip
...
% breakable-row-table: data-rows=2 total-rendered-characters=1629 max-row-rendered-characters=983
\par\medskip
\hrule height 0.8pt
\smallskip
% breakable-row: 1/2
...
% breakable-row-table: data-rows=7 total-rendered-characters=2345 max-row-rendered-characters=549
\par\medskip
\hrule height 0.8pt
\smallskip
% breakable-row: 1/7
...
\begin{enumerate}
\needspace{3\baselineskip}\item \textbf{One case, one analysis:} all $e_i$ address $m$ or its $\operatorname{succ}$-lineage under a single declared analysis $A$ (hence one task $T = \operatorname{task}(A)$; two sub-episodes sharing a task but differing in tolerance, representation, or boundary do NOT compose without an explicitly declared fusion analysis);

\needspace{3\baselineskip}\item \textbf{A declared boundary:} governance names the composite actor $\alpha_{e_\Gamma}$ (pipeline, team, institution) and configuration $\vec{\mu}_{e_\Gamma}$, including precedence across sub-episode rules;

\needspace{3\baselineskip}\item \textbf{A declared fusion rule:} $\hat{p}_{e_\Gamma}$, $\operatorname{estatus}_{e_\Gamma}$, $\delta_{e_\Gamma}$ are a stated aggregation of sub-outputs (e.g., profile union with per-claim status = weakest status along its supporting path; residual ledger = merged ledgers with surviving ownership) — well-defined, not read off the last node.
...
\begin{itemize}
\needspace{3\baselineskip}\item $x_\alpha = \Omega_\alpha(m)$ — actor-α's OBSERVATION of the shared occurrence (imperfect information is divergent observation over ONE occurrence: $\Omega_\alpha(m) \neq \Omega_\beta(m)$; perfect information is the special case of full, equal observation) — the observation/occurrence distinction applied per actor;

\needspace{3\baselineskip}\item $\hat{p}(e, \alpha, T_\alpha)$ — actor-α's current inferred profile in episode $e$ under its analysis $A_\alpha$ (task $T_\alpha$), inferred FROM $x_\alpha$, never from $m$ directly;

\needspace{3\baselineskip}\item $\operatorname{GoalSchema}(\cdot)$ — a PARAMETRIC ortheme schema (the shared goal-form under role substitution); grounded instantiations $\operatorname{GoalSchema}(\alpha)$, $\operatorname{GoalSchema}(\beta)$ are DISTINCT targets even when the schema is one (schema identity ≠ grounded-ortheme identity ≠ target-profile overlap);

\needspace{3\baselineskip}\item $\mathcal{G}_{\alpha,A_\alpha} \subseteq \Pi_{A_\alpha}$ — actor-α's grounded target profile set — NORMATIVE TYPING: a SET of profiles, each member one complete profile (the task's appropriateness rendered as a profile set; targets are selected BY the task, which remains an external parameter — a target profile is not an objective and an objective is not an ortheme);

\needspace{3\baselineskip}\item $\phi(\alpha \to \beta)$ — a role/perspective isomorphism between $\Pi_{A_\alpha}$ and $\Pi_{A_\beta}$ where one exists (goal-schema invariance with disjoint extensions: $\mathcal{G}_{\beta,A_\beta} = \phi(\mathcal{G}_{\alpha,A_\alpha})$, possibly $\mathcal{G}_\alpha \cap \mathcal{G}_\beta = \emptyset$);

\needspace{3\baselineskip}\item $\operatorname{Conflict}_m(\mathcal{G}_\alpha, \mathcal{G}_\beta)$ / $\operatorname{Compat}_m(\mathcal{G}_\alpha, \mathcal{G}_\beta)$ — WELL-TYPED across profile spaces: the two target sets live in different spaces ($\mathcal{G}_\alpha \subseteq \Pi_{A_\alpha}$, $\mathcal{G}_\beta \subseteq \Pi_{A_\beta}$), so they are never compared by bare set intersection. With $\operatorname{Reach}(m)$ the occurrences reachable from $m$ in the successor structure:
...
\begin{enumerate}
\needspace{3\baselineskip}\item \textbf{One shared concrete occurrence.} Both players face the SAME orthemma — the game state. Multi-actor cases do not multiply occurrences; they multiply perspectives on one occurrence lineage.

\needspace{3\baselineskip}\item \textbf{Actor/analysis-relative evaluation can oppose.} ``Correct for A'' and ``correct for B'' genuinely diverge under a competitive task; the theory's per-actor analysis indexing ($A_\alpha$ with $T_\alpha = \operatorname{task}(A_\alpha)$, $\Pi_{A_\alpha}$) was built for exactly this and handles it. (Notation update, D1 reconciliation: $A$ is reserved for the declared analysis; the formal actor indices are $\alpha, \beta$, with the prose labels ``Player A / Player B'' naming the actors $\alpha, \beta$ respectively. Multi-actor evaluation is a multi-analysis context, so the task-relative shorthand of manuscript Definition 3 is forbidden here and ground truth is written in full, $O^*(m; A_\alpha)$.)

\needspace{3\baselineskip}\item \textbf{Structural isomorphism across actors.} A's and B's reasoning can be the same schema under role substitution while their targets are materially incompatible — a real and clarifying observation.

\needspace{3\baselineskip}\item \textbf{Coupled episodes through a shared successor.} A's move-episode produces the successor occurrence B's episode consumes: the typed-edge episode graph (core-formalization §5.2) over one occurrence lineage.

\needspace{3\baselineskip}\item \textbf{Competitive vs cooperative as target-set relations.} Pair programming = shared target profile; chess = disjoint target profiles. Correct.
...
\begin{itemize}
\needspace{3\baselineskip}\item $x_\alpha = \Omega_\alpha(m)$ — actor-$\alpha$'s \textbf{observation} of the shared occurrence $m$ (imperfect information: $\Omega_\alpha(m) \neq \Omega_\beta(m)$ over one $m$);

\needspace{3\baselineskip}\item $\hat{p}(e, \alpha, T_\alpha)$ — the current inferred profile of actor $\alpha$'s episode $e$ under its analysis $A_\alpha$, task $T_\alpha = \operatorname{task}(A_\alpha)$ (inferred from $x_\alpha$, never from $m$ directly);

\needspace{3\baselineskip}\item $\operatorname{GoalSchema}(\cdot)$ — a \textbf{parametric} ortheme schema, the shared goal-form under role substitution (win-for-$\alpha$);

\needspace{3\baselineskip}\item $\mathcal{G}_{\alpha,A_\alpha} \subseteq \Pi_{A_\alpha}$ — actor-$\alpha$'s grounded \textbf{target profile set}: the profiles $T_\alpha$ declares appropriate as outcomes — the schema's instantiation at $\alpha$. Normative typing: a \emph{set} of profiles, each member one complete profile;

\needspace{3\baselineskip}\item $\phi(\alpha \to \beta)$ — a role/perspective isomorphism between $\Pi_{A_\alpha}$ and $\Pi_{A_\beta}$, when one exists (schema invariance: $\mathcal{G}_{\beta,A_\beta} = \phi(\mathcal{G}_{\alpha,A_\alpha})$);

\needspace{3\baselineskip}\item $\operatorname{Compat}_m(\mathcal{G}_\alpha, \mathcal{G}_\beta)$ — $\exists\, m' \in\allowbreak{} \operatorname{Reach}(m):\ O^*(m'; A_\alpha) \in\allowbreak{} \mathcal{G}_\alpha \wedge\allowbreak{} O^*(m'; A_\beta) \in\allowbreak{} \mathcal{G}_\beta$ (well-typed: one shared occurrence, each analysis evaluating in its own profile space — never bare intersection of sets from different spaces);

\needspace{3\baselineskip}\item $\operatorname{Conflict}_m(\mathcal{G}_\alpha, \mathcal{G}_\beta)$ — no such reachable $m'$ exists (cooperation: shared analysis $+$ shared target set, or an explicit alignment map $\phi(\alpha \to \beta)$).

\end{itemize}
\textbf{Three distinct relations the parametric schema untangles:} (i) SCHEMA identity — players A and B run the same parametric form $\operatorname{GoalSchema}(\cdot)$; this is the true content of the source notes' ``locally identical'' intuition; (ii) GROUNDED-ORTHEME identity — $\operatorname{GoalSchema}(\alpha)$ and $\operatorname{GoalSchema}(\beta)$ are DISTINCT grounded targets, never identical in a strictly competitive task; (iii) COMPATIBLE-OCCURRENCE overlap — pull each target profile set back along its own map $m\prime \mapsto O^*(m\prime; A_\alpha)$ or $m\prime \mapsto O^*(m\prime; A_\beta)$ to the common reachable-occurrence domain. The intersection of these pullbacks may be empty (opposed win-targets), partial, or total (cooperation). Local similarity is (i); material exclusivity is emptiness at (iii); no level asserts identity of the players' orthemes.
...
\begin{itemize}
\needspace{3\baselineskip}\item \textbf{Core formalization:} §5.3 there (multi-actor episodes over one lineage; the definitions above; explicitly derived, not a new addition). Integrated.

\needspace{3\baselineskip}\item \textbf{Terminology benchmark:} a multi-actor fixture — a scenario where the arm must keep apart: shared occurrence / actor-relative target / current profile / objective / policy / metaorthemic distinction. The source notes become the NEGATIVE calibration example for objective-vs-metaortheme-vs-policy conflation (joining the standing objective-mislabeled-as-metaortheme example). Integrated into the designed (unrun) protocol.

\needspace{3\baselineskip}\item \textbf{Revised manuscript:} §10 worked example (chess) with C1–C6 corrections — present in \texttt{orthemma-\allowbreak{}ortheme-\allowbreak{}systems-\allowbreak{}revised-\allowbreak{}draft.\allowbreak{}md}.

\needspace{3\baselineskip}\item \textbf{Operational product lane:} \textbf{no new issue.} After ordinary-language translation (``two agents with opposed success criteria acting on one artifact'') no product invariant emerges that identity, success-surface, and handoff governance do not already cover; competitive multi-agent execution is out of product scope. Default expectation confirmed: theory + benchmark only.
