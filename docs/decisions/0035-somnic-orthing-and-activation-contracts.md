...
Activation contracts are versioned governing artifacts, not skills and not keyword triggers. Each accepted version declares required properties, positive and exclusion indicators, counterexamples, ambiguity policy, plural fallbacks plus a no-claim option, effective revision, fixture outcomes, and authorship. The theological, database-schema, and residual-recurrence assessability contracts each carry exact positive, negative-near-boundary, indeterminate, and overlap evidence under a separately versioned evaluator. The bootstrap artifacts resolve to immutable authoring records; later versions require a normal authoring orthing. Meta-orthability gates resolve the exact contract, evaluator, evidence IDs, property partition, exclusions, and non-assessment disposition rather than repeating reference-shaped fields. Required-property findings remain distinct from indicators. One claimant's indeterminacy need not make the episode unresolved when another claimant or an explicitly governed baseline route is applicable.
...
V0 is limited to controlled residual-fingerprint recurrence. A match is a structural recurrence candidate, not causation, a shared defect, independence, or a remedy. Support resolves through typed episode, session, orthing, actor, source-family, normalized-input-family, and occurrence-time records. Thresholds require distinct orthings and distinct episodes; the opportunity denominator is backed by explicit opportunity IDs. The normalized fingerprint is bound to the supporting input family, and the independence result is evaluated under a fixed declared policy rather than an ad hoc subset of dimensions. Reports use “distinct episodes” unless that policy passes. A threshold is a review trigger only. Pattern equivalence, suspected locus, causal diagnosis, and intervention remain separate. The assessment surface preserves metaortheme adequacy, activation-contract adequacy, evaluator accuracy, metaorthemma binding correctness, execution fidelity, evidence adequacy, placement correctness, and closure adequacy.
