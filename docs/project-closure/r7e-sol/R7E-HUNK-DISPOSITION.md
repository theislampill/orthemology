...
The range changes exactly ten paths. Dispositions are limited to `keep`,
`revise`, `drop`, and `provenance-only`.
...
| `companion/dynamic-orthing-noetic-learning-and-orthability.md` | `revise` | Preserve the two-scale warning; revise the gap/falsifiability prose, stale modality language, notation collision, convergence wording, and doctrinal boundary. |
...
1. G1 `adopted` to `proposed-candidate`: **keep**.
2. “Locally sound” to an admissible governed corrective transition: **keep**.
3. Corrected G1 verdict: **revise** at predicate level so admissibility,
   pathway adequacy, result correctness, and strict soundness stay distinct.
4. Divergence/curl require an explicit multi-node target field: **revise** to
   use a typed operator contract and directly resolvable provenance.
5. Literal model optimization does not transfer to DAEE route pressure:
   **revise** to state Baum-Welch, Viterbi, BPTT, and Adam precisely.
6. Dynamic modalities and metaphysical rungs are different scales: **keep**.
7. Rung-3-to-rung-4 gap invariance: **revise** to avoid presenting a
   philosophical extension as empirical or already established.
